...
| 058_uva_kennzahlen | kz_044 (10% USt) + kz_057 (RC domestic §19) on ea_uva_perioden — **kz_057 wurde durch Migration 093 in `kz_032` umbenannt** (korrekte FinanzOnline-U30-Kennzahl für § 19 Abs. 1d UStG) |
...
| 085_member_email_change | `member_email_change_verifications` table (Magic-Token, `expires_at`, partieller Unique-Index auf offene Änderungen pro Mitglied) — Self-Service-E-Mail-Änderung im Mitgliederportal |
| 086_energy_imbalance_threshold | `energy_imbalance_threshold_promille` on eegs (Default 1 ‰) — Schwellwert für die Erkennung von Bezug-/Einspeise-Ungleichgewicht |
| **087_onboarding_reminder_toggle** *(Fork, Patch 0005)* | `onboarding_reminder_enabled` on eegs — schaltet die 72h-Onboarding-Erinnerungen pro EEG ab; Auto-Flip bei gesetztem `registriert_seit` |
| **088_app_login_tokens** *(Fork, Patch 0004)* | `app_login_tokens` table — kurzlebige Login-Tokens für den GrünLicht-App-QR; gespeichert wird nur der SHA-256-Hex |
| **089_member_demo_flag** *(Fork, Patch 0008)* | `is_demo` on members + partieller Unique-Index — Demo-Mitglied für den Apple-Review-Zugang |
| **090_app_login_token_code** *(Fork, Patch 0009)* | `code_hash` on app_login_tokens (+ partieller Unique-Index) — manueller 8-Zeichen-Code als Alternative zum QR |
| **091_app_login_token_source** *(Fork, Patch 0018)* | `source` on app_login_tokens (`qr` \| `email`, Default `qr`) — kennzeichnet E-Mail-Pairing-Tokens; die GrünLicht-API bestätigt nur `source='email'` automatisch |
| 092_rename_ec_req_pt *(Upstream, war 087)* | Benennt `EC_REQ_PT` → `CR_REQ_PT` in eda_processes, eda_messages und pending jobs (kanonischer ebutilities.at-Name; analog zu Migration 054) |
| 093_rename_kz057_to_kz032 *(Upstream, war 088)* | `ea_uva_perioden.kz_057` → `kz_032` — Reverse-Charge-Steuerschuld gehört im FinanzOnline U30 in Kennzahl 032 |
| 094_uva_kz016 *(Upstream, war 089)* | `kz_016` on ea_uva_perioden — Kleinunternehmer-Umsätze (§ 6 Abs. 1 Z 27 UStG) sind als Teilmenge von KZ 000 zu melden, auch in einer UVA, die nur wegen einer Reverse-Charge-Steuerschuld (KZ 032) abgegeben wird. XML-Export: neues `STEUERFREI`-Element (nur `KZ016`) zwischen `KZ000` und `VERSTEUERT` |
| 095_ea_finanzamt_iban *(Upstream, war 090)* | `ea_finanzamt_iban` (varchar 34) on eegs — IBAN der Finanzamt-Dienststelle für den SEPA-Finanzamtszahlungs-Export (Purp/Cd=TAXS) |

### Migrationsnummern im Fork — stehende Regel

Der Fork fügt eigene Migrationen (087–091, via `patches/`) in denselben Nummernraum ein wie Upstream. Bringt ein Upstream-Release neue Migrationen mit, kollidieren die Nummern und golang-migrate verweigert wegen doppelter Version den Start.

**Regel bei jedem Vendor-Drop:** Neue Upstream-Migrationen bekommen Nummern **oberhalb** der höchsten bereits in Produktion verbuchten Migration (`SELECT version FROM schema_migrations`) — nicht die eigenen nach unten schieben. Grund: golang-migrate merkt sich nur *eine* Versionsnummer. Alles darunter gilt als erledigt und würde **stillschweigend übersprungen** — die Migration liefe nie, während der neue Code das geänderte Schema schon erwartet. So wurden beim Drop 2026-07-13 Upstreams 087/088 zu 092/093 und beim Drop 2026-07-30 Upstreams 089/090 zu 094/095.

Der Nummernkonflikt ist **strukturell und wiederholt sich bei jedem Drop**: Upstream zählt ab 087 weiter, der Fork hat 087–091 belegt. Erwartungshaltung für den nächsten Drop: Upstream liefert 091/092/… und alles davon muss über den dann aktuellen Produktionsstand gehoben werden.
